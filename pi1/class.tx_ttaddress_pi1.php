...
class tx_ttaddress_pi1 extends tslib_pibase {
	...
	function main($content, $conf) {
		...
		//deutsche umlaute @ingop
		$sortBy =  str_replace('ä','ae',$sortBy);
		$sortBy =  str_replace('ö','oe',$sortBy);
		$sortBy =  str_replace('ü','ue',$sortBy);
		$sortBy =  str_replace('ß','ss',$sortBy);
	...
	function getGroupsForAddress($address) {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query(
			...
			'tt_address_group_mm',
			'tt_address_group',
			'AND tt_address.uid = '.intval($address['uid']).' AND tt_address_group.parent_group != 0'.
				$this->cObj->enableFields('tt_address'),
			'',
		...
		$groupList = implode('<br /> ', $groupTitles);
	...
	function getItemMarkerArray($address) {
		...
		$markerArray['###DESCRIPTION###']  = $lcObj->stdWrap(nl2br($address['description']),        $lConf['description.']);
		$markerArray['###MAINGROUP###']    = $lcObj->stdWrap($address['groups'][0]['title'], $lConf['mainGroup.']);
		$markerArray['###GROUPLIST###']    = $lcObj->stdWrap(str_replace('Laehrer','', $address['groupList']), 			$lConf['groupList.']);
		...
		if(!empty($address['image'])) {
			...
			for($i = 0; $i < 6; $i++) {
				...
				//skalierung des Bildes auf 120px breite
				$iConf['file.']['width'] = '120';
		...
		//kein foto in der db
				...
				$iConf['file'] = 'fileadmin/templates/medien/img/dummy-s.jpg';
				//skalierung des Bildes auf 120px breite
				$iConf['file.']['width'] = '120';
				$iConf['titleText'] = 'Noch kein Foto von '.$address['name']. ' vorhanden';
	...
	function checkSorting($sortBy) {
		...
		//keine ahnung warum, aber von nun an werden die Eintraege korrekt nach Nachnamen sortiert
		//@date 2009-07-11 ingo pfennigstorf i.pfennigstorf@fh-wolfenbuettel.de
		$sortBy = 'last_name';
